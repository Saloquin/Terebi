...
class AniListService {
    private buildHeaders(): Record<string, string> {
        ...
        const { anilistAccessToken } = userDataService.getSettings();
    ...
    private async query<T>(query: string, variables: Record<string, unknown>): Promise<T> {
        ...
        if (response.status === 401) {
            throw new AniListApiError(
                'Authentification AniList refusée. Vérifiez vos identifiants dans les paramètres.',
                401
            );
        }

        if (!response.ok) {
            throw new AniListApiError(`AniList API error: ${response.status}`, response.status);
        }
