...
class FilmCatalogService {
    ...
    async search(search: string = '', page: number = 1): Promise<CatalogResult> {
        ...
        if (normalizedSearch) {
            sourceUrl = `${this.baseUrl}/catalogue/?type%5B%5D=Film&langue%5B%5D=VOSTFR&search=${encodeURIComponent(normalizedSearch)}&page=${normalizedPage}`;
        } else {
            sourceUrl = `${this.baseUrl}/catalogue/?type%5B%5D=Film&langue%5B%5D=VOSTFR&page=${normalizedPage}`;
        }

        console.log(`🎥 Film Catalog Search: ${sourceUrl}`);
...
export const filmCatalogService = new FilmCatalogService();
