...
void main() {
  ...
  group('SettingsRepository', () {
    ...
    test('get retourne defaultValue quand la clé est absente', () async {
      final v = await repo.get('nonexistent', defaultValue: 'vostfr');
      expect(v, equals('vostfr'));
    });

    test('set puis get retourne la valeur persistée', () async {
      await repo.set(SettingsKeys.playbackLanguage, 'vf');
      final v = await repo.get(SettingsKeys.playbackLanguage);
      expect(v, equals('vf'));
    });
    ...
    test('delete supprime la clé (get retourne null après)', () async {
      await repo.set(SettingsKeys.themeMode, 'dark');
      await repo.delete(SettingsKeys.themeMode);
      final v = await repo.get(SettingsKeys.themeMode);
      expect(v, isNull);
    });

    test('plusieurs clés coexistent sans interférence', () async {
      await repo.set(SettingsKeys.playbackLanguage, 'vf');
      await repo.set(SettingsKeys.themeMode, 'light');

      expect(await repo.get(SettingsKeys.playbackLanguage), equals('vf'));
      expect(await repo.get(SettingsKeys.themeMode), equals('light'));
    });

    test('watchWithPrefix emet a chaque ecriture de cle prefixee', () async {
      final emissions = <Map<String, String>>[];
      final sub =
          repo.watchWithPrefix('anime_sama_watched:5:').listen(emissions.add);
    ...
    test('renameKeyPrefix renomme les cles old->new en conservant la valeur',
        () async {
      await repo.set('anime_sama_watched:-9:1', '4');
      await repo.set('anime_sama_watched:-9:2', '7');
      await repo.renameKeyPrefix(
          'anime_sama_watched:-9:', 'anime_sama_watched:123:');
